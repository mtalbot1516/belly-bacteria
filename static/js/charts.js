...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var sampleFiltered = sampleData.filter(sampleObj => sampleObj.id == sample);
    ...
    //  so the otu_ids with the most bacteria are last. 
      // Object.entries()
    var yticks = otu_ids.slice(0,10).reverse().map(map=> "OTU " + map)
    ...
    var barData = [{
      ...
      y: yticks,
      name: otu_labels.slice(0,10).reverse(),
    ...
    var washf = parseFloat(metaData1.wfreq)
    console.log(typeof washf)
